...
inputs:
  - .em5/infra/forge/{ticket}/spec.md (status: approved)
outputs:
...
- Lê `.em5/infra/setup/forge-templates/agent.template.md`
- Substitui placeholders com valores da spec
- Salva em `.em5/agents/{slug}/persona.md`

### 4. Criar tasks (se na spec)

Pra cada task:
- Lê `.em5/infra/setup/forge-templates/task.template.md`
- Preenche placeholders
- Salva em `.em5/agents/{agent}/{task-name}.md`

### 5. Criar playbook (se na spec)

- Lê `.em5/infra/setup/forge-templates/playbook.template.md`
- Preenche placeholders
- Salva em `.em5/agents/{nicho-ou-canal-nome}/persona.md|.em5/agents/{nicho-ou-canal-nome}/persona.md`
...
YAMLs em `.em5/system/data/{arquivo}.yaml` com schema documentado.

### 8. Inicializar learnings

Cria stubs em `.em5/system/learnings/{ano-mes}/{slug-agente}-{tipo}.md` (Art. X).

### 9. Atualizar configs centrais

- `.claude/CLAUDE.md` — adiciona linha na tabela de agentes + skills
- `.em5/system/AGENTS.md` — adiciona linha na tabela de agentes
- `em5-config.yaml` — adiciona em `agents:` se for agente novo

### 10. Build-log

Salva `.em5/infra/forge/{ticket}/build-log.md` com:
