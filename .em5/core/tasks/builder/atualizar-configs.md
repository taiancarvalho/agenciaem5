...
outputs:
  ...
  - .em5/AGENTS.md (atualizado se componente=agente)
...
1. **`.claude/CLAUDE.md`** — append linha na tabela "Agentes disponíveis":
   ...
   ```
2. **`.em5/AGENTS.md`** — append linha na tabela com Layer e função detalhada
...
Não atualiza configs centrais (task é interno do agente). Apenas garante que está em `.em5/agents/{agente}.md` na lista `commands:`.

### Se componente = playbook

Não atualiza configs centrais. `@strategist Vera` consulta `.em5/playbooks/` automaticamente.
