...
**Produto distribuído via npm:** `npx agencyos-core install`

Inspirado no padrão AIOX (`.agencyos/agents/aiox-master.md`), mas voltado para operar uma agência, não desenvolver software. O instalador replica automaticamente agentes, tasks, templates, workflows, checklists e toda a estrutura da agência no projeto do usuário.
...
O AgencyOS é um **pacote NPM publicável** — como o AIOX. Qualquer agência instala com:
...
Próximos passos:
  1. Ative um agente: @agency-master, @cs, @estrategista, @gestor-trafego
  2. Configure sua agência: @agency-master *setup-agencia
  3. Adicione um cliente: @agency-master *new-client {nome}
```
...
### Instalador (`bin/agencyos.js`)

O instalador é **o coração do produto**. Deve:

1. **Detectar estado:** greenfield (projeto novo) vs brownfield (já tem `.agencyos/`)
...
3. **Copiar estrutura** `.agencyos/` para o projeto do usuário
...
7. **Pós-instalação:** sugerir `@agency-master *setup-agencia`
...
│       ├── setup-tecnico/
│       └── operacao/
├── .agencyos/bin/                             ❌ PENDENTE — ver Tarefa 8
│   ├── agency-os-init.js
...
|--------|---------|-------|-------|
| agency-master | Atlas 🗺️ | Orquestrador geral, health do sistema | Flow 0 (Setup) |
| cs | Sol ☀️ | Onboarding, setup técnico, gestão do cliente | Flows 1, 1.5, 4 |
| estrategista | Vera 🎯 | Estratégia, posicionamento, plano de campanha | Flow 2 + Sales Optimizer |
| gestor-trafego | Max 📊 | Tráfego pago, otimização, relatórios | Flow 3 + CRO + Growth |
...
| qa-campanha | Zara 🔍 | Validação de tudo antes de publicar | Gate obrigatório |
...
**Onde criar:** `.agencyos/core/tasks/{nome-do-agente}/`
...
#### Tasks do CS (Sol) — `.agencyos/core/tasks/cs/`

| Arquivo | O que faz |
|---------|-----------|
| `iniciar-onboarding.md` | Coleta dados iniciais do cliente, cria pasta em `.agencyos/clientes/{nome}/` |
| `coletar-briefing.md` | Faz as perguntas de briefing ao usuário (elicit=true), estrutura respostas |
| `gerar-briefing-final.md` | Consolida respostas em `.agencyos/clientes/{nome}/onboarding/briefing-final.md` |
...
#### Tasks do Estrategista (Vera) — `.agencyos/core/tasks/estrategista/`
...
| `criar-plano-estrategico.md` | Cria `.agencyos/clientes/{nome}/estrategia/plano-estrategico.md` completo |
...
#### Tasks do Gestor de Tráfego (Max) — `.agencyos/core/tasks/gestor-trafego/`
...
#### Tasks do Copywriter (Cal) — `.agencyos/core/tasks/copywriter/`
...
#### Tasks do Designer (Lux) — `.agencyos/core/tasks/designer/`
...
#### Tasks do QA (Zara) — `.agencyos/core/tasks/qa-campanha/`
...
#### Tasks do Agency Master (Atlas) — `.agencyos/core/tasks/agency-master/`

| Arquivo | O que faz |
|---------|-----------|
| `criar-pasta-cliente.md` | Cria estrutura completa de pastas para novo cliente |
| `listar-clientes.md` | Lista todos os clientes em `.agencyos/clientes/` com status |
...
**Onde criar:** `.agencyos/core/templates/{categoria}/`

**Formato:** Markdown com frontmatter YAML + estrutura de preenchimento

#### `.agencyos/core/templates/briefing/`
...
#### `.agencyos/core/templates/estrategia/`
...
#### `.agencyos/core/templates/relatorios/`
...
#### `.agencyos/core/templates/contratos/`
...
#### `.agencyos/core/templates/setup-tecnico/`
...
#### Workflows — `.agencyos/core/workflows/`
...
#### Checklists — `.agencyos/core/checklists/`
...
**Onde criar:** `.agencyos/bin/`

#### `.agencyos/bin/agency-os-init.js`

Instalador do sistema AgencyOS. Deve:
1. Verificar se está rodando dentro de um projeto (git ou pasta válida)
2. Criar estrutura de pastas `.agencyos/core/` se não existir
3. Verificar se `.claude/CLAUDE.md` já tem configuração AgencyOS
...
7. Sugerir próximo passo: `node .agencyos/bin/agency-os-new-client.js`

#### `.agencyos/bin/agency-os-new-client.js`

Criador de novo cliente. Deve:
1. Perguntar nome do cliente (slug sem espaços, ex: `clinica-beleza`)
2. Verificar se pasta já existe em `.agencyos/clientes/`
3. Criar estrutura completa de pastas baseada em `.agencyos/core/data/estrutura-pastas.yaml`
4. Criar arquivos iniciais vazios com frontmatter:
   - `.agencyos/clientes/{nome}/operacao/log-operacional.md`
   - `.agencyos/clientes/{nome}/operacao/log-performance-criativa.md`
   - `.agencyos/clientes/{nome}/operacao/log-growth.md`
...
Ative um agente usando `@nome-do-agente` ou lendo o arquivo `.agencyos/agents/{nome}.md`.

| Comando | Agente | Quando Usar |
|---------|--------|-------------|
| @agency-master | Atlas 🗺️ | Setup do sistema, novo cliente, status geral |
| @cs | Sol ☀️ | Onboarding, setup técnico, gestão do relacionamento |
| @estrategista | Vera 🎯 | Estratégia, posicionamento, plano de campanha |
| @gestor-trafego | Max 📊 | Campanhas pagas, otimização, relatórios |
| @copywriter | Cal ✍️ | Copy, ângulos, adaptação por canal |
| @designer | Lux 🎨 | Criativos, vídeos, UGC, layout visual |
| @qa-campanha | Zara 🔍 | Validação antes de publicar qualquer coisa |
```
...
```
.agencyos/agents/aiox-master.md     ← padrão de agente orquestrador
.agencyos/agents/dev.md             ← padrão de agente executor
.agencyos/agents/qa.md              ← padrão de agente de qualidade
```
...
1. **Constitution** — `.agencyos/core/constitution.md` (8 artigos Não-negociaveis)
...
4. **Todos os 7 agentes** — `.agencyos/agents/` (agency-master, cs, estrategista, gestor-trafego, copywriter, designer, qa-campanha)
5. **Estrutura de pastas** — `.agencyos/clientes/_template/` (estrutura canonica do cliente)
...
1. **Installer CLI com wizard:** refatorar scripts para `bin/agencyos.js` com inquirer.js (selecao de canais, IDE, modulos)
2. **Configuração MCP:** opcional — Meta Ads, Google Ads
3. **install-manifest.yaml:** rastreamento do que foi instalado
4. **Upgrade path:** `npx agencyos-core@latest install --upgrade`
...
Fase 3: Installer CLI com wizard + Publicar `agencyos-core` no NPM
...
2. Os agentes ja construidos estao em `.agencyos/agents/`
3. A constitution esta em `.agencyos/core/constitution.md`
