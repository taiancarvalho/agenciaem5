...
// Cria workspace de novo cliente no AgencyOS
// Uso: node .agencyos/bin/agency-os-new-client.js {nome-do-cliente}
...
if (!projectName) {
  console.error('Uso: node .agencyos/bin/agency-os-new-client.js {nome-do-cliente}');
  console.error('Exemplo: node .agencyos/bin/agency-os-new-client.js clinica-sao-paulo');
  process.exit(1);
}
...
const clientesDir = path.join(rootDir, '.agencyos', 'clientes');

// Verificar se pasta AgencyOS existe
if (!fs.existsSync(path.join(rootDir, '.agencyos'))) {
  console.error('❌ AgencyOS nao encontrado neste projeto.');
  console.error('Execute primeiro: node .agencyos/bin/agency-os-init.js');
  process.exit(1);
}
...
if (fs.existsSync(clienteDir)) {
  console.error(`❌ Cliente '${slug}' ja existe em .agencyos/clientes/`);
  process.exit(1);
}
...
// Confirmar criacao
console.log(`
✅ Workspace criado: .agencyos/clientes/${slug}/
