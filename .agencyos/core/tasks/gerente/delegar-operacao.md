...
Ler o arquivo do workflow em `.agencyos/core/workflows/{nome}.yaml`.

### 2. Verificar estado do cliente

Checar em `.agencyos/clientes/{nome}/operacao/` o que já foi feito:
...
Arquivo de referência: .agencyos/clientes/{nome}/{caminho}
...
- `@cs` entrega `briefing-final.md` → `@estrategista` começa
- `@estrategista` entrega `plano-estrategico.md` → `@gestor-trafego` começa
- Todos entregam → `@qa-campanha` valida → `@gestor-trafego` publica
