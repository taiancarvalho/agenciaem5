...
**OMG.sys (One Man Gang)** é um framework operacional para agências de marketing digital, distribuído como **pacote NPM** instalável via `npx omgsys-core@latest install`.
...
```
omgsys-core/
├── package.json          ← name: "omgsys-core", bin: "omgsys"
├── bin/omgsys.js         ← CLI installer
├── .omgsys/              ← Framework completo
├── .claude/              ← Agents + commands
└── README.md
```

### Installer (`npx omgsys-core@latest install`)

1. Wizard interativo (`/setup`): nome da agência, serviços, agentes, Composio API Key
2. Gera `omgsys-config.yaml` personalizado
...
| `omgsys-config.yaml` template | ✅ |
| `system-onboarding.md` | ✅ |
| NPM wizard interativo | 🔄 Fase 3 |

---

*OMG.sys BUILD-GUIDE v1.0*
