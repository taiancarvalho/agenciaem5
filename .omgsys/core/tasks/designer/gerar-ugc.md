...
inputs:
  ...
  - .omgsys/clientes/{nome}/assets/ (imagens de referencia do produto)
outputs:
...
- **Copy**: `.omgsys/clientes/{nome}/copy/CR-{id}.md` — hook, angulo, promessa, CTA
- **Branding**: `.omgsys/clientes/{nome}/branding/` — cores, fontes, guia de estilo
- **Assets**: `.omgsys/clientes/{nome}/assets/` — fotos do produto, logo, referencias
...
Salvar JSON adaptado em `.omgsys/clientes/{nome}/design/criativos/prompts/`:
```
CR-{id}-ugc-{tipo}-prompt.json
```

Salvar a imagem em `.omgsys/clientes/{nome}/design/criativos/`:
```
CR-{id}-v1.png (ou .jpg)
```

Salvar o video em `.omgsys/clientes/{nome}/design/videos/`:
```
CR-{id}-v1.mp4
```
