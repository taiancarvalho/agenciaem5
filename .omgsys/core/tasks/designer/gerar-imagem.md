...
inputs:
  - .omgsys/clientes/{nome}/copy/CR-{id}.md (brief visual do Copywriter)
  - .omgsys/clientes/{nome}/branding/ (cores, fontes, guia de estilo)
  - .omgsys/clientes/{nome}/assets/ (logo, fotos, produtos)
outputs:
  - .omgsys/clientes/{nome}/design/criativos/CR-{id}-v1.{ext}
...
Acessar `.omgsys/clientes/{nome}/branding/`:
...
Acessar `.omgsys/clientes/{nome}/assets/`:
...
Salvar em `.omgsys/clientes/{nome}/design/criativos/`:
...
Encontrar a linha do `CR-{id}` em `.omgsys/clientes/{nome}/operacao/log-performance-criativa.md` e preencher:
