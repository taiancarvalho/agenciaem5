...
O board do cliente é o documento central do OMG.sys. Sem ele, nenhum agente consegue executar com precisão. Substitui o briefing simples: combina dados coletados com informações puxadas via Composio.
...
Confirmar que `.omgsys/clientes/{nome}/` existe. Se não, criar:
```
...
Criar `.omgsys/clientes/{nome}/onboarding/board-cliente.md` com todas as seções + dados técnicos do Composio + score + checklist.

Adicionar entrada inicial em `.omgsys/clientes/{nome}/memoria/notas-sessao.md`.
