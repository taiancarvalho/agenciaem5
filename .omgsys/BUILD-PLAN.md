...
Inspirado no padrão AIOX (`.omgsys/agents/aiox-master.md`), mas voltado para operar uma agência, não desenvolver software. O instalador replica automaticamente agentes, tasks, templates, workflows, checklists e toda a estrutura da agência no projeto do usuário.
...
1. **Detectar estado:** greenfield (projeto novo) vs brownfield (já tem `.omgsys/`)
...
3. **Copiar estrutura** `.omgsys/` para o projeto do usuário
...
│       ├── setup-tecnico/
│       └── operacao/
├── .omgsys/bin/                             ❌ PENDENTE — ver Tarefa 8
│   ├── agency-os-init.js
...
**Onde criar:** `.omgsys/core/tasks/{nome-do-agente}/`
...
#### Tasks do CS (Sol) — `.omgsys/core/tasks/cs/`

| Arquivo | O que faz |
|---------|-----------|
| `iniciar-onboarding.md` | Coleta dados iniciais do cliente, cria pasta em `.omgsys/clientes/{nome}/` |
| `coletar-briefing.md` | Faz as perguntas de briefing ao usuário (elicit=true), estrutura respostas |
| `gerar-briefing-final.md` | Consolida respostas em `.omgsys/clientes/{nome}/onboarding/briefing-final.md` |
...
#### Tasks do Estrategista (Vera) — `.omgsys/core/tasks/strategist/`
...
| `criar-plano-estrategico.md` | Cria `.omgsys/clientes/{nome}/estrategia/plano-estrategico.md` completo |
...
#### Tasks do Gestor de Tráfego (Max) — `.omgsys/core/tasks/traffic/`
...
#### Tasks do Copywriter (Cal) — `.omgsys/core/tasks/copywriter/`
...
#### Tasks do Designer (Lux) — `.omgsys/core/tasks/designer/`
...
#### Tasks do QA (Zara) — `.omgsys/core/tasks/qa/`
...
#### Tasks do Agency Master (Atlas) — `.omgsys/core/tasks/ceo/`

| Arquivo | O que faz |
|---------|-----------|
| `criar-pasta-cliente.md` | Cria estrutura completa de pastas para novo cliente |
| `listar-clientes.md` | Lista todos os clientes em `.omgsys/clientes/` com status |
...
**Onde criar:** `.omgsys/core/templates/{categoria}/`

**Formato:** Markdown com frontmatter YAML + estrutura de preenchimento

#### `.omgsys/core/templates/briefing/`
...
#### `.omgsys/core/templates/estrategia/`
...
#### `.omgsys/core/templates/relatorios/`
...
#### `.omgsys/core/templates/contratos/`
...
#### `.omgsys/core/templates/setup-tecnico/`
...
#### Workflows — `.omgsys/core/workflows/`
...
#### Checklists — `.omgsys/core/checklists/`
...
**Onde criar:** `.omgsys/bin/`

#### `.omgsys/bin/agency-os-init.js`

Instalador do sistema AgencyOS. Deve:
1. Verificar se está rodando dentro de um projeto (git ou pasta válida)
2. Criar estrutura de pastas `.omgsys/core/` se não existir
...
7. Sugerir próximo passo: `node .omgsys/bin/agency-os-new-client.js`

#### `.omgsys/bin/agency-os-new-client.js`

Criador de novo cliente. Deve:
1. Perguntar nome do cliente (slug sem espaços, ex: `clinica-beleza`)
2. Verificar se pasta já existe em `.omgsys/clientes/`
3. Criar estrutura completa de pastas baseada em `.omgsys/core/data/estrutura-pastas.yaml`
4. Criar arquivos iniciais vazios com frontmatter:
   ...
   - `.omgsys/clientes/{nome}/operacao/log-growth.md`
...
Ative um agente usando `@nome-do-agente` ou lendo o arquivo `.omgsys/agents/{nome}.md`.
...
```
.omgsys/agents/aiox-master.md     ← padrão de agente orquestrador
.omgsys/agents/dev.md             ← padrão de agente executor
.omgsys/agents/qa.md              ← padrão de agente de qualidade
```
...
1. **Constitution** — `.omgsys/core/constitution.md` (8 artigos Não-negociaveis)
...
4. **Todos os 7 agentes** — `.omgsys/agents/` (ceo, cs, strategist, traffic, copywriter, designer, qa)
5. **Estrutura de pastas** — `.omgsys/clientes/_template/` (estrutura canonica do cliente)
...
2. Os agentes ja construidos estao em `.omgsys/agents/`
3. A constitution esta em `.omgsys/core/constitution.md`
