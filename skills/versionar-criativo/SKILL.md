...
agent: designer
description: "Salvar nova vers\xE3o de criativo com ID e n\xFAmero de vers\xE3o corretos,\
  \ garantindo rastreabilidade"
inputs:
- "arquivo de imagem ou v\xEDdeo gerado"
- "ID da pe\xE7a (CR-{id})"
outputs:
- arquivo salvo em .agencyos/clientes/{nome}/design/criativos/ com nomenclatura correta
...
Consultar `.agencyos/clientes/{nome}/operacao/log-performance-criativa.md` para identificar o maior número de CR utilizado e definir o próximo.

### 3. Salvar na pasta correta

Para imagens:
```
.agencyos/clientes/{nome}/design/criativos/CR-{id}-v{n}.png
```

Para vídeos:
```
.agencyos/clientes/{nome}/design/videos/CR-{id}-v{n}.mp4
```

Para landing pages:
```
.agencyos/clientes/{nome}/design/landing-pages/CR-{id}-v{n}/
```

Para exports finais aprovados:
```
.agencyos/clientes/{nome}/design/exports/CR-{id}-v{n}.{ext}
```
