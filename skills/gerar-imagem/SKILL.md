...
name: gerar-imagem
agent: designer
description: "Gerar imagem est\xE1tica para criativo usando NanoBanana 2 via WaveSpeed\
  \ AI, com base na copy e no brief visual"
inputs:
- .agencyos/clientes/{nome}/copy/CR-{id}.md (brief visual do Copywriter)
- .agencyos/clientes/{nome}/branding/ (cores, fontes, guia de estilo)
- .agencyos/clientes/{nome}/assets/ (logo, fotos, produtos)
outputs:
- .agencyos/clientes/{nome}/design/criativos/CR-{id}-v1.{ext}
- "atualiza\xE7\xE3o no log de performance criativa"
elicit: true
metadata:
  openclaw:
    emoji: "\U0001F9E0"
...
Acessar `.agencyos/clientes/{nome}/branding/`:
...
Acessar `.agencyos/clientes/{nome}/assets/`:
...
Salvar em `.agencyos/clientes/{nome}/design/criativos/`:
...
Encontrar a linha do `CR-{id}` em `.agencyos/clientes/{nome}/operacao/log-performance-criativa.md` e preencher:
