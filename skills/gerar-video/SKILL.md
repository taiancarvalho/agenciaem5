...
name: gerar-video
agent: designer
description: Gerar video para criativo usando Kling 3.0 Pro via WaveSpeed AI, com
  prompt estruturado baseado na copy, branding e conceito visual
inputs:
- .agencyos/clientes/{nome}/copy/CR-{id}.md (copy da campanha)
- .agencyos/clientes/{nome}/branding/ (identidade visual)
- .agencyos/clientes/{nome}/design/conceito-visual-CR-{id}.md (conceito definido)
outputs:
- .agencyos/clientes/{nome}/design/videos/CR-{id}-v1.mp4
- JSON de prompt em .agencyos/clientes/{nome}/design/videos/prompts/CR-{id}-prompt.json
elicit: true
metadata:
  openclaw:
    emoji: "\U0001F9E0"
...
- **Copy**: `.agencyos/clientes/{nome}/copy/CR-{id}.md` — extrair hook, angulo, promessa e CTA
...
- **Assets**: `.agencyos/clientes/{nome}/assets/` — elementos disponiveis para usar
...
- Salvar o JSON do prompt em `.agencyos/clientes/{nome}/design/videos/prompts/CR-{id}-prompt.json`

### 6. Versionar e salvar

Salvar video em `.agencyos/clientes/{nome}/design/videos/`:
...
Salvar prompt JSON em `.agencyos/clientes/{nome}/design/videos/prompts/`:
