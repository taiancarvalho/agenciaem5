...
agent: designer
description: Criar nova versao de criativo baseado em feedback humano ou dados de
  performance, versionando e atualizando log
inputs:
- .agencyos/clientes/{nome}/operacao/log-performance-criativa.md (dados de performance)
- .agencyos/clientes/{nome}/design/criativos/ ou /videos/ (criativo original)
- "Feedback do QA (Zara), Gestor de Tr\xE1fego (Max) ou Copywriter (Cal)"
outputs:
...
Ler `.agencyos/clientes/{nome}/operacao/log-performance-criativa.md` e localizar o CR-{id}:
...
```
Imagem:  .agencyos/clientes/{nome}/design/criativos/CR-{id}-v{n+1}.png
Video:   .agencyos/clientes/{nome}/design/videos/CR-{id}-v{n+1}.mp4
Prompts: .agencyos/clientes/{nome}/design/criativos/prompts/CR-{id}-v{n+1}-prompt.json
```

### 7. Atualizar o log

Em `.agencyos/clientes/{nome}/operacao/log-performance-criativa.md`, na entrada do CR-{id}:
